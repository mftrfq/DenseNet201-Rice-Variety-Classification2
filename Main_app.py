...
#                 object_count = sum(stats[i, cv2.CC_STAT_AREA] >= 300 for i in range(1, num_labels))
...
#                         crop_gray = gray[y1:y1 + side, x1:x1 + side]
#                         crop_rgb = cv2.cvtColor(crop_gray, cv2.COLOR_GRAY2RGB)
#                         resized = cv2.resize(crop_rgb, (224, 224))
#                         x_input = tf.expand_dims(resized / 255.0, axis=0)
...
def is_multiple(image):
    # Convert to grayscale
    img_gray = cv2.cvtColor(image, cv2.COLOR_RGB2GRAY)
    _, binary = cv2.threshold(img_gray, 10, 255, cv2.THRESH_BINARY)
    num_labels, _, stats, _ = cv2.connectedComponentsWithStats(binary, connectivity=8)

    # Hitung komponen (tidak termasuk background)
    object_count = sum(1 for s in stats[1:] if s[4] > 100)
...
def predict_multiple(image):
    ...
    for i in range(1, num_labels):
        ...
        color = (0, 255, 0) if class_label == 'mentik' else (0, 0, 255)
        cv2.rectangle(img_box, (x, y), (x + w, y + h), color, 2)
        cv2.putText(img_box, class_label, (x, y - 5),
...
def Prediction():
    ...
    if uploaded_file is not None:
        ...
        st.image(pil_image, caption="Gambar Diupload", use_column_width=True)

        if is_multiple(img):
            st.info("Terdeteksi gambar dengan *multiple grain* (banyak biji)")
            pred_image, predictions = predict_multiple(img)
            st.image(pred_image, caption="Hasil Deteksi & Klasifikasi", use_column_width=True)

            for i, res in enumerate(predictions):
                st.write(f"Biji ke-{i+1}: {res['label']} (Confidence: {res['confidence']:.2f})")

        else:
            st.info("Terdeteksi gambar dengan *single grain* (satu biji)")
            label, confidence = predict_single(img)
            st.write(f"Prediksi: {label} (Confidence: {confidence:.2f})")

            resized = cv2.resize(img, (224, 224))
            st.image(resized, caption="Input ke Model", use_column_width=False)
